...
Because provider schemas can drift, the helper stores the full raw `provider-response.json` and uses tolerant extraction for `segments`, `chunks`, `words`, `diarization`, `text`, `language`, and `usage`.
